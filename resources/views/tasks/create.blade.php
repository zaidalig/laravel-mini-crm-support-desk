...
@section('page_title', 'Create New Assignment')
    ...
    <div class="col-12 col-lg-8">
        <div class="card border-0 shadow-sm rounded-4">
            <div class="card-header bg-white border-bottom py-3">
                <h5 class="m-0 fw-bold"><i class="fa-solid fa-list-check text-primary me-2"></i>Task parameters</h5>
            </div>
            <div class="card-body p-4">
                <form action="{{ route('tasks.store') }}" method="POST">
                        ...
                        <div class="col-12 col-md-6">
                            ...
                            <select name="ticket_id" id="ticket_id" class="form-select @error('ticket_id') is-invalid @enderror">
                                ...
                                @foreach($tickets as $ticket)
                                    <option value="{{ $ticket->id }}" {{ old('ticket_id', request('ticket_id')) == $ticket->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="title" id="title" class="form-control @error('title') is-invalid @enderror" placeholder="e.g. Design homepage wireframe draft" value="{{ old('title') }}" required>
                        @error('title')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="description" id="description" rows="3" class="form-control @error('description') is-invalid @enderror" placeholder="Scope, instructions, or specific notes...">{{ old('description') }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="d-flex justify-content-end gap-2 border-top pt-3">
                        ...
                        <button type="submit" class="btn btn-primary px-4 rounded-pill fw-semibold">Save Task</button>
                    </div>
                </form>
            </div>
        </div>
