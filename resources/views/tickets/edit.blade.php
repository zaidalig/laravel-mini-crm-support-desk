...
@section('page_title', 'Update Support Ticket')

@section('content')
<div class="mb-4">
    <a href="{{ route('tickets.show', $ticket->id) }}" class="btn btn-sm btn-outline-secondary rounded-pill">
        <i class="fa-solid fa-arrow-left me-1"></i> Back to Ticket Workspace
    </a>
</div>
    ...
    <div class="col-12 col-lg-9">
        <div class="card border-0 shadow-sm rounded-4">
            <div class="card-header bg-white border-bottom py-3">
                <h5 class="m-0 fw-bold"><i class="fa-solid fa-ticket-simple text-primary me-2"></i>Update Details for Ticket #{{ $ticket->id }}</h5>
            </div>
            <div class="card-body p-4">
                <form action="{{ route('tickets.update', $ticket->id) }}" method="POST">
                        ...
                        <div class="col-12 col-md-6">
                            ...
                            <select name="company_id" id="company_id" class="form-select @error('company_id') is-invalid @enderror" required>
                                ...
                                @foreach($companies as $company)
                                    <option value="{{ $company->id }}" {{ old('company_id', $ticket->company_id) == $company->id ? 'selected' : '' }}>
                                        {{ $company->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-12 col-md-6">
                            ...
                            <select name="client_id" id="client_id" class="form-select @error('client_id') is-invalid @enderror" required>
                                ...
                                @foreach($clients as $client)
                                    <option value="{{ $client->id }}" data-company-id="{{ $client->company_id }}" {{ old('client_id', $ticket->client_id) == $client->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="title" id="title" class="form-control @error('title') is-invalid @enderror" placeholder="e.g. Cannot access API" value="{{ old('title', $ticket->title) }}" required>
                        @error('title')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="description" id="description" rows="5" class="form-control @error('description') is-invalid @enderror" required>{{ old('description', $ticket->description) }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="row g-3 mb-3">
                        ...
                        <div class="col-12 col-md-3">
                            ...
                            <input type="date" name="due_date" id="due_date" class="form-control @error('due_date') is-invalid @enderror" value="{{ old('due_date', $ticket->due_date ? $ticket->due_date->format('Y-m-d') : '') }}">
                            @error('due_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="d-flex justify-content-end gap-2 border-top pt-3">
                        <a href="{{ route('tickets.show', $ticket->id) }}" class="btn btn-light border px-4 rounded-pill">Cancel</a>
