...
@section('page_title', 'Create Client Contact')
    ...
    <div class="col-12 col-lg-8">
        <div class="card border-0 shadow-sm rounded-4">
            <div class="card-header bg-white border-bottom py-3">
                <h5 class="m-0 fw-bold"><i class="fa-solid fa-user-plus text-primary me-2"></i>Contact Details</h5>
            </div>
                ...
                <form action="{{ route('clients.store') }}" method="POST">
                        ...
                        <select name="company_id" id="company_id" class="form-select @error('company_id') is-invalid @enderror" required>
                            ...
                            @foreach($companies as $company)
                                <option value="{{ $company->id }}" {{ old('company_id', request('company_id')) == $company->id ? 'selected' : '' }}>
                                    {{ $company->name }} ({{ $company->industry ?? 'Other' }})
                                </option>
                            @endforeach
                        </select>
                        @error('company_id')
                    ...
                    <div class="row g-3 mb-3">
                        ...
                        <div class="col-12 col-md-6">
                            ...
                            <input type="email" name="email" id="email" class="form-control @error('email') is-invalid @enderror" placeholder="e.g. john.doe@example.com" value="{{ old('email') }}">
                            @error('email')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-12 col-md-6">
                            ...
                            <input type="text" name="phone" id="phone" class="form-control @error('phone') is-invalid @enderror" placeholder="e.g. +1 (555) 012-3456" value="{{ old('phone') }}">
                            @error('phone')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-12 col-md-6">
                            ...
                            <input type="text" name="designation" id="designation" class="form-control @error('designation') is-invalid @enderror" placeholder="e.g. Project Manager, CEO" value="{{ old('designation') }}">
                            @error('designation')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="d-flex justify-content-end gap-2 border-top pt-3">
                        ...
                        <button type="submit" class="btn btn-primary px-4 rounded-pill fw-semibold">Save Contact</button>
                    </div>
                </form>
            </div>
        </div>
